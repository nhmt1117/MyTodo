...
MyTodo 是一个基于 Electron 的个人待办与循环提醒工具，提供待办列表、应用内自定义提醒、日历视图、任务归档和开机自启设置。单词悬浮窗的实现仍保留在代码中，但 v1 暂时隐藏入口。

当前版本为 **v1.0.0 发布候选**。核心功能、Windows 安装/卸载、快捷方式和依赖安全审计已验收；新版安装包中的应用内提醒、任务栏图标和开机自启仍待人工确认，详见 [发布清单](V1_RELEASE_CHECKLIST.md)。本轮只验证 Windows x64，macOS/Linux 的打包脚本不代表已测试支持。

## 使用

Windows 用户运行发布包 `MyTodo Setup 1.0.0.exe` 安装，无需安装 Node.js。关闭主窗口会隐藏到托盘；完全退出请使用托盘菜单中的“退出程序”。

左下角为设置入口。单词悬浮窗功能尚在开发，当前界面入口已隐藏。

## 功能

- 新增、编辑、删除普通待办
- 任务标题最多 80 字，备注描述最多 500 字；详情超高时仅内容区滚动
- 按截止日期或优先级排序
- 应用内提醒突出显示任务标题、内容和完整截止时间，支持延后 5 分钟
- 每日、每周、每月循环任务及应用内提醒
- 循环任务在日历中按规则展示每次实例
- 逾期任务高亮，并支持“不再提醒”
- 任务归档与恢复
...
├── index.html              # 主窗口 HTML
├── float.html              # 单词悬浮窗 HTML（入口暂时隐藏）
├── reminder.html           # 应用内提醒窗口 HTML
├── renderer/
│   ├── styles.css          # 主窗口样式
│   ├── renderer.js         # 主窗口交互逻辑
│   ├── float.css           # 单词悬浮窗样式
│   ├── float.js            # 单词悬浮窗交互逻辑
│   ├── reminder.css        # 应用内提醒窗口样式
│   └── reminder.js         # 应用内提醒窗口交互逻辑
├── assets/
...
│   ├── reminderScheduler.js # 应用内提醒调度
│   ├── windows.js          # 主窗口、托盘、悬浮窗和提醒窗
│   └── ipc.js              # IPC 接口注册
├── src/shared/
...
发布版默认将数据保存到安装目录的同级 `MyTodoData` 文件夹。例如：
...
首次升级时，如新默认目录为空，应用会将旧 `%APPDATA%\MyTodo` 内的任务、设置与备份自动迁移过去；新目录已有 MyTodo 数据时不会覆盖。可在应用的“设置”页查看当前数据位置并自行更改目录。Electron 缓存不属于应用数据，仍由系统管理。

开发模式和显式 `--user-data-dir` 参数仍使用指定的独立目录，不会写入 `MyTodoData`。
...
- 调度检查间隔约 30 秒；提醒使用独立的应用内窗口，不受 Windows 通知权限或勿扰模式限制。点击非按钮区域会恢复 MyTodo 并直接打开对应任务详情；“5 分钟后提醒”会持久化延后时间，“知道了”和右上角关闭按钮会结束当前提醒。应用完全退出后仍不会自行唤醒或作为后台系统服务运行。
